...
public class Account implements Serializable {
	...
	public void updateFromAccount(Account account) {
		...
		this.setHeight(account.getHeight());
		this.setTransferHash(account.getTransferHash());
	}

	public static Account clone(Account acc) {
		Account existing = new Account();
		existing.setCurve(acc.getCurve());
		existing.setUuid(acc.getUuidAsBytes());
		existing.updateFromAccount(acc);
		return existing;
	}
	...
	public String toString() {
		return "Account [id=" + id + ", uuid=" + Hex.toHexString(uuid) + ", balance=" + balance + " [" + getBalance().toPlainString() + "] " + ", transferHash=" + Hex.toHexString(transferChainHash) + ", height=" + height + "]";
	}
